...
lemma lemma_4_2 (p : ℕ) (hp : p > 3) (hprime : Nat.Prime p) : ∃ n > 0, p = 6 * n + 1 ∨ p = 6 * n - 1 :=
by
sorry
/-
-- This is the formal statement: for any prime number p, its only divisors are 1 and p.
theorem prime_divisors (p : ℕ) (hp : Nat.Prime p) : ∀ d : ℕ, d ∣ p → d = 1 ∨ d = p :=
by
  -- Assume `d` divides `p`.
  intro d hd,
  -- By definition of prime, the divisors of `p` are only 1 and `p`.
  exact hp.eq_one_or_self_of_dvd hd

have hmod : p % 6 = 1 ∨ p % 6 = 5,
{
  -- Cases on the remainder when p is divided by 6
  cases p % 6 with
  | 0 => exfalso; linarith,
  | 1 => exact Or.inl rfl,
  | 2 => exfalso; linarith,
...
by
  ...
  -- By definition of prime, the divisors of `p` are only 1 and `p`.
  -- exact hp.eq_one_or_self_of_dvd hd
  exact Nat.Prime.eq_one_or_self_of_dvd hp hd
